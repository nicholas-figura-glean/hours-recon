...
def _short_date(value: Any) -> str:
    """2026-02-20 -> 'Feb 20'. Falls back to the raw value if it is not a date."""
...
def _entry_label(entry: Mapping[str, Any]) -> str:
    """'Feb 20, 2026 - Awaneendra Tiwari - 1h': enough to find the row in Rocketlane."""
    person = _safe_text(entry.get("user_name") or entry.get("user_email"), 80)
    hours = entry.get("hours")
    parts = [part for part in (_short_date(entry.get("date")), person) if part]
    if isinstance(hours, (int, float)) and hours:
        rendered = f"{hours:g}".rstrip(".")
        parts.append(f"{rendered}h")
    activity = _safe_text(entry.get("activity_name"), 60)
    if activity:
...
def _record_labels(account: Mapping[str, Any]) -> Dict[str, str]:
    ...
    A bare Rocketlane project or time-entry ID tells an operator nothing, and the
    Salesforce Account ID written into externalReferenceId is equally opaque. The
    IDs stay authoritative for the write; these are for the human reading the card.
    """
    labels: Dict[str, str] = {}
    account_id = _safe_text(account.get("id"), 240)
    account_name = _safe_text(account.get("name"), 180)
    if account_id and account_name:
        labels[account_id] = account_name
    for project in account.get("projects", []):
        project_id = _safe_text(project.get("id"), 240)
        name = _safe_text(project.get("name"), 180)
        if project_id and name:
            labels[project_id] = name
        customer_id = _safe_text(project.get("customer_id"), 240)
        customer_name = _safe_text(project.get("customer_name"), 180)
        if customer_id and customer_name:
            # Distinguish the Rocketlane customer from the Salesforce account it maps to.
            labels[customer_id] = f"{customer_name} (Rocketlane customer)"
    for package in account.get("packages", []):
        opportunity_id = _safe_text(package.get("opportunity_id"), 240)
        name = _safe_text(package.get("opportunity_name") or package.get("name"), 180)
        if opportunity_id and name:
            labels[opportunity_id] = name
    for entry in account.get("entries", []):
        entry_id = _safe_text(entry.get("id"), 240)
...
def _operation(
    *,
    ...
    return {
        ...
        "field_value_labels": {
            name: known[str(value)]
            for name, value in fields.items()
            if isinstance(value, str) and str(value) in known
        },
        "status": status,
    ...
    *,
    ...
    for instance in workstream.get("instances", []):
        ...
        records.append({
            ...
            "project_labels": {
                _safe_text(project.get("id"), 240): _safe_text(project.get("name"), 180)
                for project in projects if project.get("id") and project.get("name")
            },
